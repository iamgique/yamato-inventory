...
class database:
    ...
    @classmethod
    def get_null_inventory_skus(cls):
        sql = "SELECT sku_id,product_name,color,size_variant,brand_id,selling_price,special_price,primary_cat_id FROM skus WHERE inventory_id IS NULL"
        cls.cursor.execute(sql)
        return cls.cursor.fetchall()
